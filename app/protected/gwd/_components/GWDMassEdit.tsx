...
export default function GWDMassEdit({ gwds, afes, onUpdate }: GWDMassEditProps) {
  ...
  const [localGWDs, setLocalGWDs] = useState(gwds)
  ...
  // Create a view state that ignores local changes for filtering
  const filterableGWDs = gwds.map(gwd => {
    const edits = editedCells.filter(cell => cell.gwd_id === gwd.gwd_id)
    return {
      ...gwd,
      // Only apply non-status edits for filtering
      ...Object.fromEntries(
        edits
          .filter(edit => edit.field !== 'status')
          .map(edit => [edit.field, edit.value])
      )
    }
  })

  // Filter using the original status
  const filteredGWDs = filterableGWDs.filter(gwd => {
    const matchesSearch = 
  ...
  // Map filtered GWDs back to their local state versions for display
  const displayGWDs = [...filteredGWDs]
    .map(gwd => {
      const localGWD = localGWDs.find(local => local.gwd_id === gwd.gwd_id)
      return localGWD || gwd
    })
    .sort((a, b) => {
      let comparison = 0
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Table with fixed height, scrollbars, and frozen first column */}
      <div className="border rounded-lg">
